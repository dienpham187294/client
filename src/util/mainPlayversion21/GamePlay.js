...
let ArrPictch = [
  0.9, 1, 1.1, 1.2, 0.9, 1, 1.1, 1.2, 0.9, 1, 1.1, 1.2, 0.9, 1, 1.1, 1.2, 0.9,
  1, 1.1, 1.2, 0.9, 1, 1.1, 1.2, 0.9, 1, 1.1, 1.2, 0.9, 1, 1.1, 1.2, 0.9, 1,
];

function ArrOfPeopeAppear_ReactJSX(props) {
  const [Info_StrickAnwers_Reactdata, SET_Info_StrickAnwers_Reactdata] =
    useState(["hi how are you"]);
  ...
  useEffect(() => {
    ...
    props.Total.stObj.ArrOfPeopeAppear_ReactData =
      props.ArrOfPeopeAppear_ReactData;
    ...
    props.Total.fnObj.SET_Info_StrickAnwers_Reactdata =
      SET_Info_StrickAnwers_Reactdata;
  ...
  function ADD_02(index, Total, Data_F_S_People) {
    try {
      if (Data_F_S_People[index] !== undefined) {
        ...
        ReadMessage(
          PickRandom(n.total.robotspeakfirst),
          Total.stObj.VoicePick,
          Total.stObj.rateRead,
          Total.stObj.pitchRead
        );
      } else {
        alert("Kết thúc");
      }
              ...
              <div>
                <div className="row">
                  {A_S_TopCenter1(
                    ...
                    Boqua,
                    D4_Time,
                    eS_CountPercentRight,
                    eS_CountPercentWrong
                  )}
                  {showTopLeftPart1(
                    Info_Avatar_Reactdata,
                    ShowInfoHint,
                    Info_Icon_Reactdata,
                    SET_ShowHint,
                    ShowHint
    ...
    return (
      <div className="M0_GameSence_Playing_OneShow">
        <div>
          ...
          <b>
            {" "}
            Cash: {Score} $<span style={{ color: "red" }}> - {Sai} </span>|{" "}
            <span style={{ color: "red" }}>{Boqua}</span>{" "}
          </b>
          <span>{D4_Time}</span>
        ...
        <div className="E_S_MOVE_BTN">
          {props.Move !== null && props.Move !== undefined ? (
            ...
            >
              {STATUS_E_S_MOVE ? (
                <span>
                  Move out<i className="bi bi-backspace"></i>
                </span>
              ) : (
                <span>
                  Move to<i className="bi bi-arrow-up-right-square"></i>
                </span>
              )}
            </button>
          ) : null}
