...
import data from "../../util/Data_GiaoAn/1120_GA_LOP1_IMG.json";
let i = 0;
export default function A2Words3000Practice() {
  ...
  useEffect(() => {
    try {
      ...
      let i3 = parseInt(ArrIndex[2]);
      setataShowGiaoan();
      let dataOutput = data[i1].slice(i2, i2 + i3);
...
function showGiaoan(data) {
  return (
    <div>
      ...
      {data.map((e, i) => (
        <div>
          {i + 1}. <b className="blueColor"> {e.name}</b>
          <br />
