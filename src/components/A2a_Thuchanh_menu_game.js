...
export default function Thuchanh_menu(props) {
  ...
  const [DataAll, setD0_ListMenu] = useState(null)
  ...
  useEffect(() => {
    fetch(T0_linkApi + props.ApiReq)
      ...
      .then((json) => {
        ...
        let n = currentdate.getDate() % json.data.length
        console.log(json.data[n].fileName)
        setD0_ListMenu(json.data[n].fileName)
      })
  }, [])
  ...
  return (
    <div>
      {
        DataAll !== null ?
          <button id="btnClickToPlay">
            <Link to={"/" + props.Linkto + "?id=" + DataAll}>
              <i>Bấm vào trò chơi online</i>
            </Link>
          </button>
          : null
      }
